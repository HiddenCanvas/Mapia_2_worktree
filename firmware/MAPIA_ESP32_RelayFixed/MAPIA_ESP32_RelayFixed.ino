...
void updateLCD() {
  ...
  lcd.print("%");
  int panjang = 7 + (kelembapanTerakhir >= 100.0 ? 5 : 4) + 1;
  for (int i = panjang; i < 16; i++) lcd.print(" ");
...
void handleParameterCommand(String pesan) {
  ...
  Serial.printf("[PARAM] threshold: min=%.1f max=%.1f\n", minKelembapan, maxKelembapan);
}
...
void koneksiMqtt() {
  ...
  while (!mqttClient.connected()) {
    ...
    if (mqttClient.connect(clientId.c_str(), MQTT_USER, MQTT_PASS)) {
      ...
      lcd.setCursor(0, 1); lcd.print("Retry ke-");
      lcd.print(retry);
      lcd.print("      ");
      delay(3000);
    }
