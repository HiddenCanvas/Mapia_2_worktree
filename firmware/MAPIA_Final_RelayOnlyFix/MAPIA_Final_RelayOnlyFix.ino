...
void handlePumpCommand(String pesan) {
  ...
  if (pesan == "ON") {
    modeAuto = false;
    Serial.println("[PUMP CMD] Manual override -> mode MANUAL");
    nyalakanPompa();
    kirimStatus(true);
    return;
  }
...
void koneksiMqtt() {
  ...
  while (!mqttClient.connected()) {
    ...
    if (mqttClient.connect(clientId.c_str(), MQTT_USER, MQTT_PASS)) {
      ...
      bool subPump = mqttClient.subscribe(topicPump.c_str(), 1);
      bool subMode = mqttClient.subscribe(topicMode.c_str(), 1);
      bool subParam = mqttClient.subscribe(topicParameter.c_str(), 1);

      Serial.printf("[MQTT] subscribe pump %s -> %s\n", topicPump.c_str(), subPump ? "OK" : "GAGAL");
      Serial.printf("[MQTT] subscribe mode %s -> %s\n", topicMode.c_str(), subMode ? "OK" : "GAGAL");
      Serial.printf("[MQTT] subscribe param %s -> %s\n", topicParameter.c_str(), subParam ? "OK" : "GAGAL");
...
void setup() {
  ...
  Serial.println("FIRMWARE RELAY GPIO26 TEST");
  ...
  digitalWrite(PIN_RELAY, HIGH); // OFF dulu untuk relay active LOW
